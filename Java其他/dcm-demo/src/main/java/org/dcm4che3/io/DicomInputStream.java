...
public class DicomInputStream extends FilterInputStream
    ...
    public void readNeedDataset(Attributes attrs) throws IOException{
        handler.startDataset(this);
        readFileMetaInformation();
        readNeedAttributes(attrs, UNDEFINED_LENGTH);
        attrs.trimToSize();
        handler.endDataset(this);
    }
    public void readNeedAttributes(Attributes attrs,long len) throws IOException{
        int[] tags = attrs.tags();
        List<Integer> tagList = Arrays.stream(tags).boxed().collect(Collectors.toList());
        int maxTag = tags[0];//优化1:最大Tag作为终止Tag，能提前终止流，减少传输带宽占用
        for (int i : tags) {
            maxTag = Math.max(maxTag, i);
        }
        int finalMaxTag = maxTag;
        Predicate<DicomInputStream> stopPredicate = o -> o.tag > finalMaxTag;

        boolean undeflen = len == UNDEFINED_LENGTH;
        long endPos =  pos + (len & 0xffffffffL);
        while (undeflen || this.pos < endPos) {
            try {
                readHeader(stopPredicate);//读取Tag、vr、数据length
            } catch (EOFException e) {
                if (undeflen && pos == tagPos)
                    break;
                throw e;
    ...
    private void skipAttributeIgnore() throws IOException {
        skipFully(length);
    }
